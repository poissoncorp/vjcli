...
def _debug_lines(model: VJModel, now: float) -> list[str]:
    ...
    return [
        ...
        (
            ...
            f"PRESET {model.performance_preset.upper()} "
        ...
        (
            f"SCENE {model.auto_scene.upper()} {model.auto_phase.upper()} "
            f"T{model.auto_scene_age:.1f} "
            f"P{model.auto_pressure:.2f} S{model.auto_score:.2f} "
            f"D{model.auto_contrast:.2f} L{model.auto_lift:.2f} "
...
def _lsd(model: VJModel) -> str:
    ...
    return (
        ...
        f"Y{theme.certainty:.2f} M{theme.margin:.2f} {theme.motif.upper()} "
        f"P{character.pace:.1f} I{character.impact:.1f} "
        f"G{character.grit:.1f} X{theme.motion:.1f} S{model.lsd_shift:.2f}"
    )
